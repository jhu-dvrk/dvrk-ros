function [is_lse_success, dynamic_parameters] = LSE_Model(Joint_No,...
    std_filter,...
    g_constant,...
    Is_Plot,...
    issave_figure,...
...
%               link: https://github.com/WPI-AIM/dvrk_gravity_comp/blob/master/Final_Submission/MATLAB%20code/Regressor_and_Parameter_matrix/plug_data.m
    ...
    data_path_struct_list = dir(input_data_files);
    Torques_data_list = {};
    theta_list = {};
    for i=1:size(data_path_struct_list,1)
        ...
        Torques_data_list{end+1} = Torques_data;
        theta_list{end+1} = int32(Theta);
    end
